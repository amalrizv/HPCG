function ReadHpcgDat(localDimensions, secondsPerRun, localProcDimensions) 
  ...
  for i=1:3
    localDimensions[i] =  parse(Int64,(strip(readuntil(hpcgStream, ' '))))
  ...
  secondsPerRun  = parse(Int64,(strip(readuntil(hpcgStream,' ' ))))
  ...
  for i= 1:3
    ...
    localProcDimensions[i] = parse(Int64,(strip(readuntil(hpcgStream, ' '))))
