...
#=
 Creates a YAML file and writes the information about the HPCG run, its results, and validity.
  ...
  @see YAML_Doc
=#
function ReportResults(A, numberOfMgLevels, numberOfCgSets, refMaxIters, optMaxIters, times,
    testcg_data, testsymmetry_data, testnorms_data, global_failure, quickPath) 
  ...
  t4max = 0.0
   t4avg = 0.0
  MPI.Allreduce(t4, &t4min, 1, MPI_DOUBLE, MPI_MIN, MPI_COMM_WORLD)
  MPI.Allreduce(t4, &t4max, 1, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD)
  MPI.Allreduce(t4, &t4avg, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD)
  t4avg = t4avg/((Float64)A.geom.size)
#endif
  ...
  if (A.geom.rank==0) # Only PE 0 needs to compute and report timing results
    ...
    fnops_sparsemv = (fniters+fNumberOfCgSets)*2.0*fnnz # 1 SpMV with nnz adds and nnz mults
    # Op counts from the multigrid preconditioners
    fnops_precond = 0.0
    SparseMatrix  Af = A
    for i=1:numberOfMgLevels
    ...
    fnreads_ddot = (3.0*fniters+fNumberOfCgSets)*2.0*fnrow*sizeof() # 3 ddots with 2 nrow reads
    fnwrites_ddot = (3.0*fniters+fNumberOfCgSets)*sizeof() # 3 ddots with 1 write
    fnreads_waxpby = (3.0*fniters+fNumberOfCgSets)*2.0*fnrow*sizeof() # 3 WAXPBYs with nrow adds and nrow mults
    fnwrites_waxpby = (3.0*fniters+fNumberOfCgSets)*fnrow*sizeof() # 3 WAXPBYs with nrow adds and nrow mults
    fnreads_sparsemv = (fniters+fNumberOfCgSets)*(fnnz*(sizeof()+sizeof(local_int_t)) + fnrow*sizeof())# 1 SpMV with nnz reads of values, nnz reads indices,
    # plus nrow reads of x
    #fnwrites_sparsemv = (fniters+fNumberOfCgSets)*fnrow*sizeof() # 1 SpMV nrow writes
    #Op counts from the multigrid preconditioners
    fnreads_precond = 0.0
    ...
    for i=1:numberOfMgLevels
      ...
      fnumberOfPresmootherSteps = Af.mgData.numberOfPresmootherSteps
      fnumberOfPostsmootherSteps = Af.mgData.numberOfPostsmootherSteps
      fnreads_precond += fnumberOfPresmootherSteps*fniters*(2.0*fnnz_Af*(sizeof()+sizeof(local_int_t)) + fnrow_Af*sizeof()) # number of presmoother reads
      fnwrites_precond += fnumberOfPresmootherSteps*fniters*fnrow_Af*sizeof() # number of presmoother writes
      fnreads_precond += fniters*(fnnz_Af*(sizeof()+sizeof(local_int_t)) + fnrow_Af*sizeof()) # Number of reads for fine grid residual calculation
      fnwrites_precond += fniters*fnnz_Af*sizeof() # Number of writes for fine grid residual calculation
      fnreads_precond += fnumberOfPostsmootherSteps*fniters*(2.0*fnnz_Af*(sizeof()+sizeof(local_int_t)) + fnrow_Af*sizeof())  # number of postsmoother reads
      fnwrites_precond += fnumberOfPostsmootherSteps*fniters*fnnz_Af*sizeof()  # number of postsmoother writes
      Af = Af.Ac # Go to next coarse level
    end

    fnnz_Af = Af->totalNumberOfNonzeros
    fnrow_Af = Af->totalNumberOfRows
    fnreads_precond += fniters*(2.0*fnnz_Af*(sizeof()+sizeof(local_int_t)) + fnrow_Af*sizeof()) # One symmetric GS sweep at the coarsest level
    fnwrites_precond += fniters*fnrow_Af*sizeof() # One symmetric GS sweep at the coarsest level
    ...
    fnbytes += (() sizeof()*fNumberOfCgSets) # testnorms_data in main.cpp

    # Model for GenerateProblem_ref.cpp
    fnbytes += fnrow*sizeof(char)      # array nonzerosInRow
    fnbytes += fnrow*(() sizeof(global_int_t*)) # mtxIndG
    fnbytes += fnrow*(() sizeof(local_int_t*))  # mtxIndL
    fnbytes += fnrow*(() sizeof(*))      # matrixValues
    fnbytes += fnrow*(() sizeof(*))      # matrixDiagonal
    fnbytes += fnrow*numberOfNonzerosPerRow*(() sizeof(local_int_t))  # mtxIndL[1..nrows]
    fnbytes += fnrow*numberOfNonzerosPerRow*(() sizeof())       # matrixValues[1..nrows]
    fnbytes += fnrow*numberOfNonzerosPerRow*(() sizeof(global_int_t)) # mtxIndG[1..nrows]
    fnbytes += fnrow*(() 3*sizeof()) # x, b, xexact

    # Model for CGData.hpp
     fncol = ((global_int_t) A.localNumberOfColumns) * size # Estimate of the global number of columns using the value from rank 0
    fnbytes += fnrow*(() 2*sizeof()) # r, Ap
    fnbytes += fncol*(() 2*sizeof()) # z, p

    std::vector<> fnbytesPerLevel(numberOfMgLevels) # Count byte usage per level (level 0 is main CG level)
    ...
    for (int i=1 i<numberOfMgLevels ++i) {
       fnrow_Af = Af->totalNumberOfRows
       fncol_Af = ((global_int_t) Af->localNumberOfColumns) * size # Estimate of the global number of columns using the value from rank 0
       fnbytes_Af = 0.0
      # Model for GenerateCoarseProblem.cpp
      fnbytes_Af += fnrow_Af*(() sizeof(local_int_t)) # f2cOperator
      fnbytes_Af += fnrow_Af*(() sizeof()) # rc
      fnbytes_Af += 2.0*fncol_Af*(() sizeof()) # xc, Axf are estimated based on the size of these arrays on rank 0
      fnbytes_Af += (() (sizeof(Geometry)+sizeof(SparseMatrix)+3*sizeof(Vector)+sizeof(MGData))) # Account for structs geomc, Ac, rc, xc, Axf - (minor)

      # Model for GenerateProblem.cpp (called within GenerateCoarseProblem.cpp)
      fnbytes_Af += fnrow_Af*sizeof(char)      # array nonzerosInRow
      fnbytes_Af += fnrow_Af*(() sizeof(global_int_t*)) # mtxIndG
      fnbytes_Af += fnrow_Af*(() sizeof(local_int_t*))  # mtxIndL
      fnbytes_Af += fnrow_Af*(() sizeof(*))      # matrixValues
      fnbytes_Af += fnrow_Af*(() sizeof(*))      # matrixDiagonal
      fnbytes_Af += fnrow_Af*numberOfNonzerosPerRow*(() sizeof(local_int_t))  # mtxIndL[1..nrows]
      fnbytes_Af += fnrow_Af*numberOfNonzerosPerRow*(() sizeof())       # matrixValues[1..nrows]
      fnbytes_Af += fnrow_Af*numberOfNonzerosPerRow*(() sizeof(global_int_t)) # mtxIndG[1..nrows]

      # Model for SetupHalo_ref.cpp
#ifndef HPCG_NO_MPI
      fnbytes_Af += (() sizeof()*Af->totalToBeSent) #sendBuffer
      fnbytes_Af += (() sizeof(local_int_t)*Af->totalToBeSent) # elementsToSend
      fnbytes_Af += (() sizeof(int)*Af->numberOfSendNeighbors) # neighbors
      fnbytes_Af += (() sizeof(local_int_t)*Af->numberOfSendNeighbors) # receiveLength, sendLength
#endif
    ...
    # This final GFLOP/s rating includes the overhead of problem setup and optimizing the data structures vs ten sets of 50 iterations of CG
     totalGflops = frefnops/(times[0]+fNumberOfCgSets*(times[7]/10.0+times[9]/10.0))/1.0E9
     totalGflops24 = frefnops/(times[0]+fNumberOfCgSets*times[7]/10.0)/1.0E9
