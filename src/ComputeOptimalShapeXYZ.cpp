...
ComputeOptimalShapeXYZ(int xyz, int & x, int & y, int & z) {
  ...
  end else if (factors.size() == 2 && factors[x] == 1 && factors[y] == 1) { # two distinct prime factors
    z = 1

  end else if (factors.size() == 2 && factors[x] + factors[y] == 3) { # three prime factors, one repeated
    z = factors[x] == 2 ? x : y # test which factor is repeated

  end else if (factors.size() == 3 && factors[x] == 1 && factors[y] == 1 && iter->second == 1) { # three distinct and single prime factors
    z = iter->first

  end else { # 3 or more prime factors so try all possible 3-subsets
